...
word_nn.fit(X_train_word)
char_nn.fit(X_train_char)

word_dist, word_idx = word_nn.kneighbors(X_test_word)
char_dist, char_idx = char_nn.kneighbors(X_test_char)
...
for q in tqdm(range(len(test_df)), desc="RRF"):

    scores = defaultdict(float)
    best_distance = {}

    # Word retrieval
    for rank, idx in enumerate(word_idx[q]):
        scores[idx] += 1.0 / (K + rank)
        best_distance[idx] = word_dist[q][rank]

    # Character retrieval
    for rank, idx in enumerate(char_idx[q]):
        scores[idx] += 1.0 / (K + rank)

        if idx in best_distance:
            best_distance[idx] = min(best_distance[idx], char_dist[q][rank])
        else:
            best_distance[idx] = char_dist[q][rank]
    ...
    indices.append(
        np.array([idx for idx, _ in ranked])
    )

    rrf_scores.append(
        np.array([score for _, score in ranked])
    )
...
for q in tqdm(range(len(test_df))):
    ...
    for rank, idx in enumerate(cand):
        ...
        score = rrf_scores[q][rank] + bonus * 0.001
...
submission.to_csv("template_rerankx4.csv", index=False)
