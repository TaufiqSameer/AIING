...
train_tokens = [doc.lower().split() for doc in train_text]
test_tokens = [doc.lower().split() for doc in test_text]
...
for q in range(len(test_df)):
    mp = {}
    for r, idx in enumerate(indices[q]):
        mp[idx] = distances[q][r]
    distance_lookup.append(mp)

for tokens in tqdm(test_tokens, desc="BM25"):

    scores = bm25.get_scores(tokens)

    top = np.argpartition(scores, -150)[-150:]
...
for q in tqdm(range(len(test_df))):
    ...
    for rank, idx in enumerate(cand):
        ...
        if idx in distance_lookup[q]:
            base = -distance_lookup[q][idx]
        elif idx in bm25_rank[q]:
            base = (150 - bm25_rank[q][idx]) * 0.0005
        else:
            base = -1
