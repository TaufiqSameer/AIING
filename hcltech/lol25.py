...
train_tail = [get_tail(x) for x in train_text]
test_tail  = [get_tail(x) for x in test_text]
...
tail_vectorizer = TfidfVectorizer(
    stop_words="english",
    ngram_range=(1,3),
    sublinear_tf=True,
    min_df=2,
...
X_train_tail = tail_vectorizer.fit_transform(train_tail)
X_test_tail = tail_vectorizer.transform(test_tail)
...
tail_nn = NearestNeighbors(
    metric="cosine",
    algorithm="brute",
    n_neighbors=150,
)

tail_nn.fit(X_train_tail)

tail_dist, tail_idx = tail_nn.kneighbors(X_test_tail)
final_indices = []
...
for q in range(len(test_df)):
    mp = {}
    for r, idx in enumerate(tail_idx[q]):
        mp[idx] = r
    tail_rank.append(mp)
...
for q in tqdm(range(len(test_df))):
    ...
    for rank, idx in enumerate(cand):
        ...
        if idx in tail_rank[q]:
            tail_bonus = (150 - tail_rank[q][idx]) * 0.00005
...
submission.to_csv("gg1.csv", index=False)
