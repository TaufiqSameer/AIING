...
suffix_vectorizer = TfidfVectorizer(
    stop_words="english",
    ngram_range=(1,3),
    binary=False,
    sublinear_tf=True,
...
X_suffix_train = suffix_vectorizer.fit_transform(suffix_train)
X_suffix_test = suffix_vectorizer.transform(suffix_test)
...
for q in tqdm(range(len(test_df))):
    ...
    base_score = 1.0 - distances[q]

    suffix_sim = cosine_similarity(
        X_suffix_test[q],
        X_suffix_train[candidates]
    ).ravel()
